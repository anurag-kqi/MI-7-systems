...
void write_society_D(struct society socty)
{
  ...
  /*society insertion*/
void insert_society_D(struct society_disk soc_data)
{
    //FILE *fp;
    //fp = fopen("society.txt", "a+");

    struct society *newNode = malloc(sizeof(struct society));
    strcpy(newNode->soc.owner_name, soc_data.owner_name);
    newNode->soc.flat_num = soc_data.flat_num;
    newNode->soc.owner_contact = soc_data.owner_contact;
    newNode->next = NULL;
    newNode->prev = NULL;
